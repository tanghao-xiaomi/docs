...
The Framebuffer driver framework of openvela provides the [interface](../../../../../../nuttx/blob/trunk-5.2/drivers/video/fb.c) needed for `poll`, to monitor whether the Framebuffer is in a writable state:
...
Using semaphores for synchronization is equivalent to locking the Framebuffer. The renderer must acquire the lock each time it begins rendering; otherwise, it will remain in a blocked state. Please refer to this [link](../../../../../../nuttx/blob/trunk-5.2/arch/arm/src/stm32/stm32_ltdc.c) for the code.
