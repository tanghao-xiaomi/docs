...
The **oneshot** driver-related interface information is in the [oneshot.h](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/timers/oneshot.h) file, and is also divided into **Upper Half** and **Lower Half** interface layers.
...
`arch_alarm` provides a series of interfaces to meet the timer requirements of the sched module. Interface information can be found in the [arch.h](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/arch.h) header file.
...
During the board initialization phase, it is necessary to invoke the **vendor-customized initialization function** to complete the allocation and initialization of the [struct oneshot_lowerhalf_s](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/timers/oneshot.h#L226) structure. This function is provided by the openvela framework, with the prototype as follows:
...
- Structure Definition: For details on the members of `struct oneshot_lowerhalf_s`, refer to [oneshot.h](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/timers/oneshot.h#L226). Fill in function pointers such as interrupt triggering and timer startup according to hardware characteristics.
...
Upper-Half interfaces are defined in [arch.h](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/arch.h#L1460), primarily for use by the scheduler (Sched).
...
- Vendor Selection
    ...
    - Unimplemented interface groups can be automatically mapped via openvela's built-in [conversion functions](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/timers/oneshot.h).
...
`struct oneshot_operations_s` is defined in [oneshot.h](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/timers/oneshot.h), with the following member functions.
...
- Hardware (Arch Layer) Timer Initialization, refer to code [arch/risc-v/src/bl602/bl602_timerisr.c](../../../../../../../../nuttx/blob/trunk-5.5/arch/risc-v/src/bl602/bl602_timerisr.c#L57).
...
- Oneshot Driver Instantiation, refer to code [arch/risc-v/src/bl602/bl602_oneshot_lowerhalf.c](../../../../../../../../nuttx/blob/trunk-5.5/arch/risc-v/src/bl602/bl602_oneshot_lowerhalf.c#L361).
...
The operation interface binding is as follows, and the detailed code can be referred to in [arch/risc-v/src/bl602/bl602_oneshot_lowerhalf.c](../../../../../../../../nuttx/blob/trunk-5.5/arch/risc-v/src/bl602/bl602_oneshot_lowerhalf.c#L96).
...
Detailed code can be found in [include/time.h](../../../../../../../../nuttx/blob/trunk-5.5/include/time.h#L233).
...
Applications can directly operate the Oneshot timer through the `ioctl` function. Before using this feature, the `/dev/oneshot` device node must be registered during the system startup (bringup) process. Refer to the header file [include/nuttx/timers/oneshot.h](../../../../../../../../nuttx/blob/trunk-5.5/include/nuttx/timers/oneshot.h#L41) for the currently supported `ioctl` commands. Command descriptions are as follows:
