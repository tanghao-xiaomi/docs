...
- [Deploy openvela on STM32H750 ​](#deploy-openvela-on-stm32h750-)
  - [I. Overview](#i-overview)
  - [II. Preparation](#ii-preparation)
    - [1. Set up the development environment](#1-set-up-the-development-environment)
    - [2. Connect the Development Board](#2-connect-the-development-board)
  - [III. Running the Demo](#iii-running-the-demo)
    - [1. Download the Code](#1-download-the-code)
    - [2. Compile the code](#2-compile-the-code)
    - [3. Run the program](#3-run-the-program)
  - [IV. Development Board Overview](#iv-development-board-overview)
    - [1. Appearance](#1-appearance)
    - [2. Key Hardware Features](#2-key-hardware-features)
  - [V. Appendix](#v-appendix)
    - [1. Create Development Board Configuration File Directory](#1-create-development-board-configuration-file-directory)
    - [2. Enable Double Buffering Drawing Mode](#2-enable-double-buffering-drawing-mode)
      - [2.1 Configure SDRAM to Support Double Buffering Mode](#21-configure-sdram-to-support-double-buffering-mode)
      - [2.2 Configure the LTDC Frame Buffer to SDRAM](#22-configure-the-ltdc-frame-buffer-to-sdram)
      - [2.3 Modifying the LCDC Driver to Support Double Buffering](#23-modifying-the-lcdc-driver-to-support-double-buffering)
      - [2.4 Adding the `pandisplay` Method to Trigger Display Content Updates](#24-adding-the-pandisplay-method-to-trigger-display-content-updates)
      - [2.5 Clearing the Previous Frame Data in the Reload Interrupt Callback](#25-clearing-the-previous-frame-data-in-the-reload-interrupt-callback)
      - [2.6 Verifying the Effect of Double Buffering Mode Configuration](#26-verifying-the-effect-of-double-buffering-mode-configuration)
    - [3. Configuring Demo Application to Start Automatically at Boot](#3-configuring-demo-application-to-start-automatically-at-boot)
      - [3.1 Creating the System Startup Scripts](#31-creating-the-system-startup-scripts)
      - [3.2 Configuring the Read-Only Memory File System (ROMFS)](#32-configuring-the-read-only-memory-file-system-romfs)
      - [3.3 Compiling the Startup Script into the Target File](#33-compiling-the-startup-script-into-the-target-file)
        - [Method 1: Generate `etc_romfs.c` File](#method-1-generate-etc_romfsc-file)
        - [Method 2: Directly Reference the Startup Scripts in the Makefile](#method-2-directly-reference-the-startup-scripts-in-the-makefile)
      - [3.4 Compiling and Downloading the Program](#34-compiling-and-downloading-the-program)
...
4. Select the **ST-LINK** connection method. Click the green **Connect** button. When the button changes to **Disconnect**, the development board is successfully connected, as shown in the figure below:
    ...
    > Note
    >
    > For **ST-LINK** documentation, see the [official documentation](https://www.st.com/resource/en/technical_note/tn1235-overview-of-stlink-derivatives-stmicroelectronics.pdf).
...
1. Calculate the RAM requirement for double buffering based on the screen size.
    ...
    > **Note**
    >
    > The internal Static Random Access Memory (SRAM) of the development board is insufficient, so external Synchronous Dynamic Random Access Memory (SDRAM) is required.
...
2. Modify driver parameters to support double buffering.
    ...
    1. Configure the frame buffer size.
        ...
        Modify the frame buffer size formula to support double buffering:
        ...
        > Note
        >
        > Here the height is multiplied by 2, reserving double the display area for double buffering.
...
1. Add the `pandisplay` method pointer to the virtual table.
    ...
    > Note
    >
    > This addition allows the `pandisplay` method to be used only when double buffering (`CONFIG_FB_DOUBLE_BUFFER`) is enabled.
...
2. Implement the `pandisplay` method to trigger the LCDC screen transfer.
    ...
    > Note
    >
    > This method calculates the new frame start address based on buffer parameters and calls the reload interface to ensure the new content is displayed on screen in a timely manner.
...
Refer to the [Boot Process](../../device_dev_guide/kernal/boot_process.md) documentation. To enable auto-start functionality, add the following script files to the startup process:
...
> Note
>
> This script will run automatically during the system startup phase to launch the Demo application `lvgldemo`.
