...
**代码路径**: [`nuttx/boards/arm/stm32/stm32f407-robomaster/`](./../../../../../../nuttx/tree/trunk-5.2/boards/arm/stm32/stm32f407-robomaster)
...
#ifdef CONFIG_USERLED
  /* 注册用户 LED 驱动，使其在 /trunk-5.2/userleds 节点可用 */
  ret = userled_lower_initialize("/trunk-5.2/userleds");
  if (ret < 0)
    {
...
该文件为通用的 LED 驱动（位于 `drivers/leds/userled_lower.c`）提供底层的硬件操作接口。应用程序通过标准的 `open()`, `write()`, `ioctl()` 等VFS接口访问 `/trunk-5.2/userleds` 来控制这些 LED。
