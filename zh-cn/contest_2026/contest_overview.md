...
3. **申领开发板**：申请开发板后，组委会会审核你的**作品方向与所选开发板是否匹配**（约 3 个工作日）；匹配通过后，按下方「赋能支持」缴纳 50 元押金，开发板通过**顺丰快递寄出**（约 3 个工作日发货）。提交作品者全额退还押金、开发板免费留存；未提交作品者需归还开发板，归还后押金全额退回。（纯快应用方向初赛用 openvela 模拟器即可、无需真机，真机可在决赛阶段再申领。）
4. **拿到专属仓**：组委会按 `contest2026_<编号>_<队伍名>` 为你创建 GitHub 专属仓库并开通 push 权限。仓库创建后请**及时登录 GitHub 接受协作邀请**（邀请默认 7 天内有效，过期需重新申请）。详见 [《参赛代码提交指南》](./code_submission_guide.md)。
