...
4. 安装 ST-Link 驱动。
    ...
    - 下载地址：请从 [ST 官网](https://www.st.com/en/development-tools/stsw-link007.html#get-software) 下载 `stsw-link007` 软件包并解压。
        ...
        <img src="./images/029.png" alt="stsw-link007" width="75%">
...
5. 启动后界面如下：
    ...
    <img src="./images/030.png" alt="STM32CubeProgrammer" width="75%">

### 3、硬件连接与验证

1. 查看原理图准备连接硬件。

    <img src="./images/031.png" alt="Schematic" width="75%">
...
2. 请按照下表所示，将各部件连接起来。
    ...
    <img src="./images/032.jpg" alt="Physical Connection" width="50%">
...
3. 验证连接。
    ...
    2. 在右上角的 **ST-LINK Configuration** 区域，单击**刷新**按钮。如果 **Serial number** 下拉框中出现了设备序列号，则表明调试器已被成功识别。
        ...
        <img src="./images/033.png" alt="Serial number" width="75%">

    3. 在左侧导航栏选择 **Erasing & Programming**，如下图所示：

        <img src="./images/034.jpeg" alt="Erasing  Programming" width="75%">

    4. 选择 **J-Link** 或 **STLINK** 方式，单击绿色 **Connect** 按钮。当按钮变为 **Disconnect** 时，表示开发板已成功连接，如下图所示：

        <img src="./images/035.jpg" alt="Connect" width="75%">
...
请确保您已根据官方[下载 openvela 源代码](../../zh-cn//quickstart/Download_Vela_sources_zh-cn.md)文档，在本地准备好了完整的 `openvela` 源代码。

### 2、(可选) 理解项目结构

`openvela` 的代码遵循分层设计。了解关键目录有助于您进行后续的定制开发。

- **应用层 (Examples)**：[nuttx-apps/examples/leds](../../../../../../../open-vela/nuttx-apps/tree/dev/examples/leds)
- **板级支持包 (****BSP****)**：[nuttx/boards/arm/stm32/stm32f411-minimum](../../../../../../../open-vela/nuttx/tree/dev/boards/arm/stm32/stm32f411-minimum)
...
2. 选择目标开发板。
   ...
   1. 按回车键进入 **Board Selection**，打开开发板选择菜单。
        ...
        <img src="./images/036.png" alt="Board Selection" width="75%">

   2. 按回车键选择 **Select target board**，选择要编译的目标平台:

        <img src="./images/037.png" alt="Select target board" width="75%">

   3. 按回车键选择 **STM32F411CEU6 Minimum ARM Development Board**，为 STM32F411CEU6 最小系统板编译固件:

        <img src="./images/038.png" alt="STM32F411CEU6" width="75%">

3. 启用 LED 驱动。

   1. 按两次 Esc 键返回到第一级目录，选择 **Device Drives**:

        <img src="./images/039.png" alt="Device Drives" width="75%">

   2. 按 Enter 键进入 `LED support`：

        <img src="./images/040.png" alt="LED support" width="75%">
   ...
   3. 启用以下配置项：
        ...
        - **Generic Lower Half LED Driver**
            ...
            <img src="./images/041.png" alt="LED Driver" width="75%">

4. 启用示例程序。

   1. 按 **/** 键搜索并启用 **EXAMPLES_LEDS**。

        <img src="./images/042.png" alt="EXAMPLES_LEDS" width="75%">

   2. 按 **/** 键搜索并禁用 **ARCH_LEDS**（避免冲突）。

        <img src="./images/043.png" alt="ARCH_LEDS" width="75%">
...
2. 单击 **Browse**，选择上一步生成的固件文件 **`openvela/nuttx/nuttx.hex`**，并勾选 **Skip flash erase before programming**，如下图所示：
    ...
    <img src="./images/044.png" alt="Browse" width="75%">

3. 单击 **Start Programming** 开始下载。下载完成后，会弹出提示框，并且日志窗口会输出相关信息。

    <img src="./images/045.png" alt="Start Programming" width="75%">

4. 如果下载过程中出现 **Error**，请单击 **Full chip erase** 按钮擦除芯片数据，然后重新下载固件，即可恢复正常。

    <img src="./images/046.png" alt="Full chip erase" width="75%">
...
2. （可选）如果首次使用 Minicom 的无法接收键盘输入，请参考如下步骤修改 Minicom 的相关配置：
    ...
    1. 使用以下命令打开 Minicom 配置界面：
        ...
        <img src="./images/047.png" alt="Serial port setup" width="75%">
    ...
    2. 进入 **Serial port setup** 配置目录，确保以下两项配置值为 **No**：
        ...
        - Software Flow Control: No
            ...
            <img src="./images/048.png" alt="Hardware Flow Control" width="75%">
...
1. 重新打开 minicom，连接成功后，在 Minicom 终端中按回车，您会看到 `nsh>` 提示符。
    ...
    <img src="./images/049.png" alt="nsh" width="75%">
...
1. 返回 `openvela` 根目录，运行 `menuconfig`。
...
<img src="./images/050.png" alt="undefined reference" width="75%">
...
<img src="./images/051.png" alt="userleds" width="75%">
...
<img src="./images/052.png" alt="userleds" width="75%">
