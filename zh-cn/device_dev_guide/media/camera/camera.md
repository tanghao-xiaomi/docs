...
| **imgsensor** | 通过**读写** **S****ensor** **的****寄存器**方式，实现对**具体****S****ensor** **操作**的功能。因为每个 Sensor 的寄存器配置会有差异，通过该接口可以屏蔽不同的差异。 | 调用 I2C 接口，配置 Sensor 的寄存器，实现 Sensor 图像参数设置，图像采集开启和停止控制。                           |
