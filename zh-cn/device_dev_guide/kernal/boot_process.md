...
- `board_early_initialize`：用于板级早期初始化，依赖于 CONFIG_BOARD_EARLY_INITIALIZE，执行上下文是 initialization thread 中，不能等待任何事件，不能调用任何可能阻塞的函数（比如sem_wait）。
- `board_late_initialize`：用于板级稍晚的初始化，依赖于 CONFIG_BOARD_LATE_INITIALIZE，执行上下文是内核临时的 thread 中，可以等待事件。SoC大部分驱动初始化在此过程中。
- `board_app_initialize`：用于应用初始化，通过 boardctl（BOARDIOC_INIT）被调用，执行上下文是 nsh task。注意这个阶段不能访问文件系统。
- `rc.sysinit`：第一阶段脚本，主要用于文件系统挂载和核心启动服务初始化，目前 SoC 最新代码已经将原 rcS 中文件系统挂载和核心服务（kvdb、uorb 和 usrsock）启动拆分到此脚本中，执行时机是 nsh 可输入前。
- `board_app_finalinitialize`：用于板级最终的初始化，通过 boardctl（BOARDIOC_FINALINIT）被调用，执行上下文是 nsh task。目前最新代码已经将需要文件访问的驱动初始化（TP、Charger、Audio PA、BMI、PPG 和 GPS）挪到这个里面，直接操作文件，不需要使用 delay work 的方式推后执行。
- `rcS`：第二阶段脚本，主要用于启动其他的应用程序，rumiwear、algo_service、gpsd 和 healthd 等，执行时机是 nsh 可输入前。

## 启动脚本

openvela 启动脚本 rcS、rc.sysinit 通过 nshlib 进行加载和解析，nshlib 通过指定位置 CONFIG_ETC_ROMFSMOUNTPT/CONFIG_NSH_SYSINITSCRIPT 和 CONFIG_ETC_ROMFSMOUNTPT/CONFIG_NSH_INITSCRIPT 读取启动脚本的内容。
