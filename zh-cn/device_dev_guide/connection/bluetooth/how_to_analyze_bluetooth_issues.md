...
- [发现、连接、配对问题](#发现连接配对问题)
  ...
  - [典型问题](#典型问题-1)
    ...
    - [问题：CTKD BLE LTK 生成 BR LinkKey 失败](#问题ctkd-ble-ltk-生成-br-linkkey-失败)
      - [1：打开协议栈 Debug 功能](#1打开协议栈-debug-功能)
      - [2：复现问题](#2复现问题)
      - [3：日志解读](#3日志解读)
      - [4:如何确认当前 LinkKey 是否由 CTKD 生成？](#4如何确认当前-linkkey-是否由-ctkd-生成)
    - [问题：设备通过 RPA 地址广播未建立连接](#问题设备通过-rpa-地址广播未建立连接)
      - [1:BLE 配对状态机与流程图](#1ble-配对状态机与流程图)
      - [2:设备通过 RPA 地址广播建立连接过程](#2设备通过-rpa-地址广播建立连接过程)
      - [3:确认 BLE 配对完成](#3确认-ble-配对完成)
      - [4:确认 IRK 交换成功](#4确认-irk-交换成功)
      - [5:确认通过 Identity 地址建立 BR/EDR 连接](#5确认通过-identity-地址建立-bredr-连接)
      - [6:断连/重启后回连情况](#6断连重启后回连情况)
...
- [音频传输问题](#音频传输问题)
  - [分析方法](#分析方法-2)
    ...
    - [方法：观察A2DP SRC是否停止音频流传输](#方法观察a2dp-src是否停止音频流传输)
      ...
      - [1 通过snoop log观察A2DP SRC是否停止传输音频包](#1-通过snoop-log观察a2dp-src是否停止传输音频包)
